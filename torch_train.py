...
def getsample(n=50):
  ...
  y = torch.where(torch.Tensor(labels), train_org[idxs, yn], train_forg[idxs, yn])
...
def train_step(x, y, label):
  ...
  targets = torch.where(label>0, torch.tensor([1.]), torch.tensor([-1.]))
...
def test():
  with torch.no_grad():
    ...
    embx = model(x)
    emby = model(y)
    targets = torch.where(labels>0, torch.tensor([1.]), torch.tensor([-1.]))
