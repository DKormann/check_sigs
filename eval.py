...
datapath = '/shared/datasets/signatures/small_dataset/'
...
def random_pair(datapath = datapath, ds_offset=0, ds_size = 10_000, b_size = 24):
  ...
  cp = datapath + f"/genuine/{n:03}/c-{n:03}-{c:0>2}.PNG"
  if label: fp = datapath + f"/genuine/{n:03}/c-{n:03}-{f:0>2}.PNG"
  else: fp = datapath + f"/forged/{n:03}/cf-{n:03}-{f:0>2}.PNG"
  return cp, fp, label
...
def test_sample(datapath = datapath, ds_size = 20, b_size = 24):
  ...
  for n, l in zip(n, label):
    for i in range(1,img_per_set+1):
      ...
      r.append(loadimage(datapath + f"/genuine/{n:03}/{n:03}_{i:0>2}.PNG"))
      f.append(loadimage(datapath + (f"/genuine/{n:03}/{n:03}_{i_:0>2}.PNG" if l else f"/forged/{n:03}/{n:03}_{i:0>2}.PNG")))
